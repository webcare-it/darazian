...
Route::group(['prefix' => 'v2/auth'], function() {
    ...
    Route::post('password/check_verification_code', 'Api\V2\PasswordResetController@checkVerificationCode');
...
Route::group(['prefix' => 'v2'], function() {
    ...
    Route::get('image-path/{id}', 'Api\V2\GeneralSettingController@imagePath')->middleware('api.cache');
    ...
    Route::get('products/todays-deal', 'Api\V2\ProductController@todaysDeal')->middleware('api.cache');
    Route::get('products/featured', 'Api\V2\ProductController@featured')->middleware('api.cache');
    Route::get('products/best-seller', 'Api\V2\ProductController@bestSeller')->middleware('api.cache');
    ...
    Route::post('update-shipping-cost-in-cart', 'Api\V2\CartController@updateShippingCostInCart');
    ...
    Route::get('policies/seller', 'Api\V2\PolicyController@sellerPolicy')->name('policies.seller')->middleware('api.cache');
    Route::get('policies/support', 'Api\V2\PolicyController@supportPolicy')->name('policies.support')->middleware('api.cache');
    Route::get('policies/return', 'Api\V2\PolicyController@returnPolicy')->name('policies.return')->middleware('api.cache');
...
//    Route::post('shipping_cost', 'Api\V2\ShippingController@shipping_cost')->middleware('auth:sanctum');
    Route::get('shipping-cost', 'Api\V2\ShippingController@shipping');

    Route::post('coupon/apply', 'Api\V2\CouponController@apply')->middleware('auth:sanctum');

    Route::get('/combined-order/{id}','Api\V2\OrderController@combined_order');
    Route::get('track-order/{order_code}', 'Api\V2\OrderController@track_order');
    ...
    // Order OTP routes
    Route::post('order/send-otp', 'Api\V2\OrderOtpController@sendOtp');
    Route::post('order/verify-otp', 'Api\V2\OrderOtpController@verifyOtp');
    ...
    Route::get('customer/dashboard-summary/{id}', 'Api\V2\CustomerDashboardController@dashboard_summary')->middleware('auth:sanctum');
    ...
    Route::post('subscribers/store', 'Api\V2\SubscriberController@store');
    Route::delete('subscribers/destroy/{id}', 'Api\V2\SubscriberController@destroy');
