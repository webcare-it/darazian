...
// Serve React SPA with dynamic meta tags injected per route
if (!function_exists('serveReact')) {
    function serveReact(string $route = 'home', string $slug = ''): string
    {
        ...
        $globalDescription = get_setting('meta_description') ?: 'Shop the latest fashion, electronics, beauty, home goods & more at unbeatable prices. ✓ Free shipping ✓ Easy returns ✓ Secure payment';
        $globalMetaImage   = get_setting('meta_image')       ? uploaded_asset(get_setting('meta_image')) : null;
        $globalOgImage     = $globalMetaImage ?: ($baseUrl . '/og-image.jpg');
        $globalFavicon     = get_setting('site_icon')        ? uploaded_asset(get_setting('site_icon'))  : null;

        // Default meta (uses admin global SEO settings as base)
        $meta = [
            'title'       => $globalTitle,
        ...
        switch ($route) {
            case 'products':
                // Check if $slug contains "id/name" — single product detail page
                // The name segment is generated by frontend slugify(), not stored in DB — use ID only
                if ($slug && str_contains($slug, '/')) {
                    [$productId, $nameSegment] = array_pad(explode('/', $slug, 2), 2, '');
                    $product = Product::find((int) $productId, ['id', 'name', 'meta_title', 'meta_description', 'thumbnail_img']);
                    if ($product) {
                ...
                // Fallback: product listing page
                $meta['title']       = 'All Products - ' . $appName;
                $meta['description'] = 'Browse all products on ' . $appName . '. Best prices, free shipping.';
                $meta['url']         = $baseUrl . '/products';
                break;

            case 'categories':
                $label               = $slug ? ucwords(str_replace('-', ' ', $slug)) : 'All Categories';
                $meta['title']       = $label . ' - ' . $appName;
                $meta['description'] = 'Browse ' . strtolower($label) . ' on ' . $appName . '.';
                $meta['url']         = $baseUrl . '/categories' . ($slug ? '/' . $slug : '');
                break;

            case 'campaign':
                if ($slug) {
                    $landing = \App\Models\LandingPage::where('slug', $slug)
                        ->select('title', 'short_description', 'banner_image', 'slug')
                        ->first();
                    if ($landing) {
            ...
            case 'brands':
                $meta['title']       = 'Brands - ' . $appName;
                $meta['description'] = 'Explore all brands available on ' . $appName . '.';
                $meta['url']         = $baseUrl . '/brands';
                break;

            case 'flash-deal':
                $meta['title']       = 'Flash Deals - ' . $appName;
                $meta['description'] = 'Limited time flash deals on ' . $appName . '. Grab the best discounts now!';
                $meta['url']         = $baseUrl . '/flash-deal';
                break;

            case 'cart':
                $meta['title']       = 'Your Cart - ' . $appName;
                $meta['description'] = 'Review your cart and checkout on ' . $appName . '.';
                $meta['url']         = $baseUrl . '/cart';
                break;

            case 'checkout':
                $meta['title']       = 'Checkout - ' . $appName;
                $meta['description'] = 'Complete your purchase on ' . $appName . '.';
                $meta['url']         = $baseUrl . '/checkout';
                break;

            case 'signin':
                $meta['title']       = 'Sign In - ' . $appName;
                $meta['description'] = 'Sign in to your ' . $appName . ' account.';
                $meta['url']         = $baseUrl . '/signin';
                break;

            case 'signup':
                $meta['title']       = 'Create Account - ' . $appName;
                $meta['description'] = 'Create a new account on ' . $appName . '.';
                $meta['url']         = $baseUrl . '/signup';
                break;

            case 'search':
                $q                   = request()->query('q', '');
                $meta['title']       = $q ? ('Search: ' . $q . ' - ' . $appName) : ('Search - ' . $appName);
                $meta['description'] = 'Search results on ' . $appName . '.';
                $meta['url']         = $baseUrl . '/search' . ($q ? '?q=' . urlencode($q) : '');
                break;
        }
        ...
        if (get_setting('facebook_domain_verification') && get_setting('fb_domain_code')) {
            $facebookDomainVerification = '<meta name="facebook-domain-verification" content="' . htmlspecialchars(get_setting('fb_domain_code')) . '" />' . "\n";
        }
        ...
        ' . $faviconTag . $facebookDomainVerification;
