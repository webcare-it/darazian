...
class BusinessSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            if ($type == 'timezone') {
                ...
                // 🔥 Update Category and Brand "top" column based on selection
                if ($type == 'top10_categories') {
                    $selectedCategories = $request->top10_categories ?? [];
                    Category::query()->update(['top' => 0]); // reset all
                    Category::whereIn('id', $selectedCategories)->update(['top' => 1]);
                }

                if ($type == 'top10_brands') {
                    $selectedBrands = $request->top10_brands ?? [];
                    Brand::query()->update(['top' => 0]); // reset all
                    Brand::whereIn('id', $selectedBrands)->update(['top' => 1]);
                }
    ...
    public function shipping_configuration(Request $request){
        $shipping_costs = ShippingCost::latest()->get();
        return view('backend.setup_configurations.shipping_configuration.index', compact('shipping_costs'));
    }
